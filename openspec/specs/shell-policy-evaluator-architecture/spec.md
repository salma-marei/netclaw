# shell-policy-evaluator-architecture Specification

## Purpose

Define the internal structure of the shell authorization evaluator. One
call-local state holds candidates, coverage, grant evidence, and trace facts.
The policy phases run in one fixed order and fail closed. This capability keeps
the evaluator auditable and holds its observable decisions constant across
refactor work.
